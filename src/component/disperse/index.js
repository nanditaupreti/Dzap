...
const Disperse = () => {
  ...
  const validateLine = (line, lineNumber) => {
    ...
    if (parts.length === 2 && !isNaN(Number(parts[1]))) {
      ...
      if (uniqueAddresses[address]) {
        if (!uniqueAddresses[address].includes(lineNumber)) {
          uniqueAddresses[address].push(lineNumber);
        }
      } else {
      ...
      return `Line ${lineNumber}: Invalid amount or format`;
    }
  };
  ...
  const onSubmit = () => {
    ...
    const duplicateLines = Object.entries(uniqueAddresses)
      .filter(([address, lineNumbers]) => lineNumbers.length > 1)
      .map(([address, lineNumbers]) => `Duplicate address '${address}' found in Lines: ${lineNumbers.join(', ')}`);
    if (duplicateLines.length > 0) {
      setDuplicatesEncountered(true);
      setResult([...validationResults, ...duplicateLines]);
    }
  };
  ...
  const keepFirstOne = () => {
    const uniqueLines = [];
    const uniqueAddresses = {};
    setDuplicatesEncountered(false);
  ...
  return (
    <div style={{ margin: 'auto', width: '35%' }}>
      ...
      <p style={{ textAlign: 'initial', color: 'grey' }}>Separated by ',' or '' or '='</p>
      <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
              ...
              <div onClick={combineBalances}>
                <p style={{ color: 'red', cursor: 'pointer' }}>Combine Balance</p>
              </div>
            </div>
          </div>
        )}
